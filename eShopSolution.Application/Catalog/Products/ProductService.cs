            ...
            try
            {
                var query = from p in _context.Products
                            ...
                            from c in cpic.DefaultIfEmpty()
                                //join ct in _context.CategoryTranslations on c.Id equals ct.CategoryId into ctc
                                //from ct in ctc.DefaultIfEmpty()
                            where pt.LanguageId == languageId && p.IsFeturead == true
                            select new { p, pt, pic };

                var data = await query.OrderByDescending(x => x.p.DateCreated).Take(take).Select(x => new ProductVm()
                {
                    Id = x.p.Id,
                    Name = x.pt.Name,
                    DateCreated = x.p.DateCreated,
                    ...
                    ThumbnailImage = (from pi in _context.ProductImages
                                      where pi.ProductId == x.p.Id && pi.isDefault == true
                                      select pi.ImagePath)
                                      .FirstOrDefault()
                }).ToListAsync();
